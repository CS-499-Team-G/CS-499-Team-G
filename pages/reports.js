...
function outgoingShipmentTable(data) {
	...
	for (let i = 0; i < data.length; i++) {
		...
		table.rows[i + 1].cells[7]
			...
			.setAttribute(
				"onclick",
				"console.log(this.parentNode.parentNode.id); sessionStorage.setItem('objID', this.parentNode.parentNode.id); window.open(this.href, 'Shipment Manifest', 'width=500, height=500, left=100, top=100, scrollbars, resizable'); return false;"
			);
	}
}
